...
window.login = async function() {
  ...
  try {
    ...
    // Nombres personalizados por usuario
...
function cargarMenuLateral(email){
  ...
  menu.forEach(depto => {
    ...
    li.addEventListener("click", () => {
      ...
      // Oculta el menú lateral automáticamente al seleccionar
...
function initChat() {
  ...
  chatListener = onSnapshot(q, snapshot => {
    ...
    snapshot.forEach(docSnap => {
      ...
      // Puntos de estado (rojo=entregado, verde=visto)
      const puntos = esPropio ? 
